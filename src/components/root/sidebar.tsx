...
export default function Sidebar({ isCollapsed, isMobileOpen, onCloseMobile }: SidebarProps) {
  ...
  const sidebarClasses = `
    fixed inset-y-0 inset-s-0 z-50 md:sticky md:top-0 h-dvh flex flex-col bg-background-card border-r border-border transition-all duration-300 w-64
  ...
  return (
    <aside className={sidebarClasses}>
      {/* Brand Header */}
      <div className={`h-16 flex items-center border-b border-border shrink-0 select-none ${isSidebarExpanded ? "justify-between px-6" : "justify-center px-0"
        }`}>
        ...
        {isMobileOpen && (
          <button
            ...
            className="p-2 -me-2 rounded-lg hover:bg-hover text-secondary hover:text-foreground md:hidden cursor-pointer border-none bg-transparent"
          >
            <XIcon size={22} />
          </button>
        )}
        ...
        {menuItems.map((item) => {
          ...
          return (
            <Link
              ...
              to={item.path}
              onClick={onCloseMobile}
              className={`flex items-center rounded-xl transition-all select-none py-3 ${isSidebarExpanded
        ...
        {(userPermissionNames.includes("users.read") || userPermissionNames.includes("permissions.read")) && (
          <div>
            <button
              ...
              className={`flex items-center w-full rounded-xl transition-all select-none py-3 cursor-pointer border-none bg-transparent ${isSidebarExpanded
            ...
            >
              {userPermissionNames.includes("users.read") && (
                <Link
                  ...
                  onClick={onCloseMobile}
                  className={`flex items-center gap-2.5 rounded-lg py-2 px-3 text-xs font-semibold transition-all select-none ${location.pathname === "/settings/manage-users"
              ...
              {userPermissionNames.includes("permissions.read") && (
                <Link
                  ...
                  onClick={onCloseMobile}
                  className={`flex items-center gap-2.5 rounded-lg py-2 px-3 text-xs font-semibold transition-all select-none ${location.pathname === "/settings/manage-permissions"
                    ? "bg-primary/20 text-primary"
                    : "text-secondary hover:text-foreground hover:bg-hover"
                    }`}
                >
